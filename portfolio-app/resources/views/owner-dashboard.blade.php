...
<div class="main_page" id="main_page">
    ...
    <div class="admin-card">
        <h3>Manage Course Progress</h3>

        <form action="{{ route('owner.courses.bulkUpdate') }}" method="POST">
            @csrf
            @method('PATCH')
                ...
                <div class="workflow-item">
                    <div class="workflow-details">
                        <div style="display: flex; gap: 10px; width: 100%;">
                            <input type="text" name="courses[{{ $course->id }}][course_name]" class="form-control" value="{{ $course->course_name }}" style="font-weight: bold;" required>
                        </div>
                        <div style="display: flex; gap: 10px; width: 100%;">
                            <input type="number" name="courses[{{ $course->id }}][credits_ec]" class="form-control" value="{{ $course->credits_ec }}" style="width: 80px;" required>
                            <input type="number" name="courses[{{ $course->id }}][grade]" class="form-control" value="{{ $course->grade }}" placeholder="Grade" style="width: 80px;">
                        </div>
                    </div>

                    <div class="workflow-actions">
                        <select name="courses[{{ $course->id }}][status]" class="form-control" style="width: auto; padding: 5px 10px; height: 38px;">
                        ...
                        <button type="button" class="btn-danger" onclick="if(confirm('Delete {{ $course->course_name }}?')) { document.getElementById('delete-course-{{ $course->id }}').submit(); }">
                            Delete
                        </button>
                    </div>
                </div>
        ...
        @foreach($courses as $course)
        <form id="delete-course-{{ $course->id }}" action="{{ route('owner.course.destroy', $course) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
        @endforeach
        ...
        <form action="{{ route('owner.course.store') }}" method="POST">
            ...
            <button type="submit" class="btn-submit">Log Course</button>
        </form>
    </div>
